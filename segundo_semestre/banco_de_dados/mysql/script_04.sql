-- CRIE UMA FUNÇÃO PARA RETORNAR A SIGLA DO ESTADO ATRAVÉS DO ID
...
CREATE TABLE estado (
id int(11) NOT NULL,
nome varchar(75) DEFAULT NULL,uf varchar(2) DEFAULT NULL,ibge int(2) DEFAULT NULL,pais int(3) DEFAULT NULL,ddd varchar(50) DEFAULT NULL
...
CREATE FUNCTION retornar_sigla(id_estado INT)
RETURNS VARCHAR(2)
RETURN (SELECT uf FROM estado WHERE id = id_estado);

DELIMITER $$
CREATE OR REPLACE FUNCTION retornar_sigla(id_estado INT)
RETURNS CHAR(2)
BEGIN
	DECLARE uf_estado CHAR(2);
    SELECT uf INTO uf_estado FROM estado WHERE id = id_estado;
    RETURN uf_estado;
END $$
DELIMITER ;

SELECT retornar_sigla(13);
